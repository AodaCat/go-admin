...
func (v *viewAuthorityMenuMysql) Initialize() error {
	...
	sql = strings.ReplaceAll(sql, "@menus", "sys_base_menu")
	sql = strings.ReplaceAll(sql, "@authorities_menus", entity.TableName())
	if err := global.GA_DB.Exec(sql).Error; err != nil {
...
func (v *viewAuthorityMenuMysql) CheckDataExist() bool {
	err1 := global.GA_DB.Find(&[]system.SysMenu{}).Error
	err2 := errors.New(fmt.Sprintf("Error 1146: Table '%v.%v' doesn't exist", global.GA_CONFIG.Mysql.Dbname, v.TableName()))
