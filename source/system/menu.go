...
func (m *menu) Initialize() error {
	...
	if err := global.GA_DB.Create(&entities).Error; err != nil { // 创建 model.User 初始化数据
...
func (m *menu) CheckDataExist() bool {
	return !errors.Is(global.GA_DB.Where("path = ?", "autoCodeEdit/:id").First(&system.SysBaseMenu{}).Error, gorm.ErrRecordNotFound)
}
