...
type Timer interface {
	...
	AddTaskByJob(taskName string, spec string, job cron.Job) (cron.EntryID, error)
...
func (t *timer) AddTaskByJob(taskName string, spec string, job cron.Job) (cron.EntryID, error) {
	t.Lock()
	defer t.Unlock()
	if _, ok := t.taskList[taskName]; !ok {
		t.taskList[taskName] = cron.New()
